...
describe('On mounted.vue', () => {
  ...
  test('When calling handleImage change', () => {
    const wrapper = shallowMount(CreateRuin, {
      global: { plugins: [store, router] },
    });
    wrapper.vm.handleImageChange();
    jest.spyOn(wrapper.vm, 'handleImageChange');
    expect(wrapper.vm.handleImageChange).toHaveBeenCalled();
  });
